...
const mem_t eeprom_mem[] = {
    ...
    { .default_uint32 = 0, .type = uint32_type, .idx = idx_filp_mode, .pData = &eeprom.flip_mode, .addr = offsetof(eeprom_t, flip_mode), .size = sizeof(uint32_t), .p_check = &eeprom_block_check_buf[idx_filp_mode] },
